...
static void g_wildcard_proxy_resolver_set_property(
        GObject *object,
        guint prop_id,
        const GValue *value,
        GParamSpec   *pspec)
...
static void g_wildcard_proxy_resolver_get_property(
        GObject *object,
        guint prop_id,
        GValue *value,
        GParamSpec *pspec)
...
static void g_wildcard_proxy_resolver_class_init(
        GWildcardProxyResolverClass *resolver_class)
{
...
static gchar** g_wildcard_proxy_resolver_lookup(
        ...
        GError **error)
{
    ...
    for (int i = 0; i < resolver->priv->regex_wildcard->len; ++i)
    {
        GRegex *regex_wildcard = (GRegex*)g_ptr_array_index(
                    resolver->priv->regex_wildcard, i);
        if (g_regex_match(regex_wildcard, uri, (GRegexMatchFlags)0, NULL))
        {
            Proxy *p = (Proxy*)g_ptr_array_index(
                        resolver->priv->proxies, i);
...
static void g_wildcard_proxy_resolver_lookup_async(
        GProxyResolver *proxy_resolver,
        const gchar *uri,
        GCancellable *cancellable,
        ...
        gpointer user_data)
{
    ...
    proxies = g_wildcard_proxy_resolver_lookup(proxy_resolver, uri,
                                            cancellable, &error);
    if (proxies)
...
static gchar** g_wildcard_proxy_resolver_lookup_finish(
...
static void g_wildcard_proxy_resolver_iface_init(
